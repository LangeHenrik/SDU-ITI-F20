...
class Feed extends Database {
	...
	public function getImageUserApi($id){
		...
		$sql = "SELECT id, header AS title, description, user_id, created FROM user INNER JOIN images ON user.id_user = images.user_id WHERE id_user = :id ORDER BY created DESC";
